...
class _PublishedRideDetailsScreenState
  ...
  void _setupMap() {
    ...
    try {
      ...
      final bounds =
          _locationService.getBoundsForMultipleLocations(allLocations);
  ...
  void _startRide() async {
    ...
    try {
      ...
      if (currentPosition != null) {
        ...
        final locationDoc =
            await FirebaseFirestore.instance.collection('liveLocations').add({
        ...
        for (var doc in acceptedPassengers.docs) {
          batch.update(doc.reference, {
            ...
            'pickupLocationName':
                _rideData!['from']?['name'] ?? 'Pickup location'
          });
        }
        await batch.commit();
...
// Helper method to send FCM notification
  Future<void> _sendRideStartedNotification(
      String fcmToken, String locationDocId, String pickupLocation) async {
    try {
            ...
            child:
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    _buildDetailRow(
                        'From', _rideData!['from']?['name'] ?? 'Unknown'),
                    const SizedBox(height: 8),
                    _buildDetailRow(
                        'To', _rideData!['to']?['name'] ?? 'Unknown'),
                    const SizedBox(height: 8),
                    _buildDetailRow(
                        'Date', _rideData!['date'] ?? 'Not specified'),
                    const SizedBox(height: 8),
                    _buildDetailRow(
                        'Time', _rideData!['time'] ?? 'Not specified'),
                    const SizedBox(height: 8),

                    // Show intermediate stops if any
                    if (_rideData!.containsKey('intermediatePoints') &&
                        _rideData!['intermediatePoints'] is List &&
                        (_rideData!['intermediatePoints'] as List)
                            .isNotEmpty) ...[
                      const Divider(),
                      const Text(
                        'STOPS',
                    ...
                    // Route details
                    if (_rideData!.containsKey('routeDistance') &&
                        _rideData!.containsKey('routeDuration')) ...[
                      _buildDetailRow(
                        'Distance',
                      ...
                      children: [
                        Expanded(
                          child: _buildDetailRow(
                            'Price',
                            'Sepolia eth ${_rideData!['amount'] ?? "0"}',
                          ),
                        ),
                        Expanded(
                          child: _buildDetailRow(
  ...
  String _getVehicleDescription() {
    if (_rideData!.containsKey('vehicleDetails')) {
      final vehicleDetails =
          _rideData!['vehicleDetails'] as Map<String, dynamic>?;
      if (vehicleDetails != null) {
        ...
        if (model.isNotEmpty)
          description += description.isNotEmpty ? ' ($model)' : model;
        if (plate.isNotEmpty)
          description += description.isNotEmpty ? ' - $plate' : plate;
